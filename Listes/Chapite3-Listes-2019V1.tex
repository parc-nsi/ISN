...
\begin{introduction}{}
Pour traiter des données complexes comme des images on a besoin d'un type de variable plus élaboré qu'un simple entier (\lstinline{<class 'int'>}). Il faudra, dans une unique variable, pouvoir stocker un nombre représentant le niveau de gris de chaque élément de l'image, c'est un tableau d'entiers dont on a besoin. En Python, les listes permettent de faire cela.
\end{introduction}  

  
 \section{Définitions et opérations sur les listes}
 
 \begin{definition}{(Listes)}
Les listes de Python correspondent à une structure de données généralement appelée tableaux.

Elles consistent en un ensemble d'objets de types quelconques (éventuellement dépareillés) ordonnés et accessibles par leur numéro de position appelé indice ou index. Attention le premier élément porte le numéro 0. On dit que c'est une structure de données de type composite.
 \end{definition}

Pour définir explicitement une liste à partir de ses éléments on utilise les délimiteur \lstinline{[} et \lstinline{]} et le séparateur \lstinline{,}. Si il n'y a pas d'élèment, on obtient la liste vide.
\bigskip
...
On peut définir une liste de listes\footnote{C'est une bonne façon de représenter un tableau à double entrée.} ou même une liste de listes de listes, etc. La fonction \lstinline{len} renvoie le nombre d'élément d'une liste.
\bigskip
...
Pour accéder à un élément, on utilise son indice. \bcattention{} Attention, les indices commencent à 0.
\bigskip
...
Pour supprimer un élément on utilise l'instruction \lstinline{del} (delete signifie effacer en anglais).
\bigskip
...
On peut modifier une liste en changeant la valeur d'un de ces éléments\footnote{On dit qu'une liste est mutable ou modifiable.} : 
\bigskip
...
{\bf \Large Intermède : notion d'objets et de méthode en Python}
\vskip 0.5cm

Python est un langage orienté objet; tout ce qu'on manipule est objet : une variable, une liste, une  fonction sont des objets. Un objet est muni de \og méthodes\fg, ce sont les actions que l'on peut lui appliquer.

Pour donner une idée de ce que c'est, on pourrait dire qu'un objet de type \og chien\fg \ devrait disposer des méthodes : manger, aboyer, chercher le bâton (liste non exhaustive).

Pour appliquer une méthode de sa classe à un objet on utilise la syntaxe : 

\begin{center}
{\large \lstinline{nom_objet.nom_methode}}
\end{center}


Par exemple la méthode \lstinline{.sort()} permet de trier une liste :
\bigskip
...
>>> L
[1, 5, 7, 5, 9]
>>> L.count(5)    # renvoie le nombre d'occurences de la valeur
2
>>> L.index(5)    # renvoie l'indice de la première occurence
1
>>> L.remove(5)    # élimine la première occurence de la valeur
>>> L
[1, 7, 5, 9]
...
La fonction \lstinline{list()} renvoie une liste correspondant à son argument :
\bigskip
...
Les listes sont \emph{itérables}. On peut les parcourir avec une boucle for {\bf sans utiliser les indices }: 

\bigskip

\begin{minipage}{0.95\linewidth}
\begin{lstlisting}[style=compil]
>>> noms = ['Alice', 'Bernard', 'Cé;cile']
>>> for x in noms:
		print(x)
...
\end{minipage}
 ...
 On suppose donnée une liste d'entiers L, un entier x. \'Ecrire un code qui compte dans une variable n le nombre d'éléments de L supérieurs strictement à x.
 ...
 On suppose donnée une liste d'entiers L et un entier x. \'Ecrire un code qui renvoie la liste (éventuellement vide) des indices des occurrences de la valeur x dans la liste L.
 
 \lignes{10}
 
 \end{exercice}

\section{Concaténation et tranchage}
...
On peut découper une tranche dans une liste (la logique est celle de la fonction \lstinline{range}) :
\bigskip
...
>>> L[2:8:2]
[4, 5, 2]
>>> L[::-1]     #inversion de l'ordre d'apparition des éléments d'une liste
[3, 5, 6, 2, 9, 5, 1, 4, 1, 3]
\end{lstlisting}
\end{minipage}
...
\section{Listes par compréhension}
...
[4, 6, 8, 9, 10, 12]
 ...
 \begin{enumerate}
  \item  \'Ecrire une instruction  qui génère une liste de tous les carrés des entiers pairs  entre $0$ et $100$.
  
 \lignes{1}
 
   \item  Compléter le code ci-dessous pour que la liste \texttt{L} contienne uniquement les  images positives des entiers compris entre $0$ et $100$ par la fonction cosinus.
...
L = .............................................................
 ...
 \item  \'Ecrire un code qui produit une liste de listes \verb+tables+ qui contient les résultats des tables de multiplication entre $1$ et $9$ :
...
%\newpage
 ...
 \section{Conséquences du caractère modifiable des listes}
 
 En python, un nom ne correspond pas directement à la valeur de la variable mais elle donne la référence (qui n'est que l'adresse en mémoire) de cette valeur. Le schéma présenté dans le premier cours\footnote{Précisément en bas de la page 4.} n'est qu'une représentation générale de la notion de variable en informatique. En python on a plutôt :  
 
 
\begin{center}
 \includegraphics[width=9 cm]{variable_0.pdf}
 \end{center}
 
 
 On obtient la référence d'une variable grâce à la fonction \lstinline{id()} :
 \medskip
...
Le type entier (\lstinline{'int'}) n'est {\bf pas modifiable} ce qui signifie qu'une valeur de type entier ne peut pas être modifiée après sa création. Concrètement, voilà ce qui se passe :
...
\begin{center}
\includegraphics[width=7 cm]{variable_couple.pdf}
\end{center}
\end{minipage}
...
\begin{minipage}{0.45\linewidth} 
\begin{center}
\includegraphics[width=7 cm]{variable_couple2.pdf}
\end{center}
\end{minipage}

Lors de l'exécution de l'instruction \lstinline{b = 2*b}, il n'y a pas modification de la valeur contenue dans la mémoire\footnote{Ce serait catastrophique car la valeur de \lstinline{a} ne serait plus 5 !} à l'adresse 4297546688 mais Python crée une nouvelle valeur pour le résultat. Ensuite la référence de \lstinline{b} est modifiée (4297546848) pour pointer vers cette valeur.
...
>>> c = b//2
>>> id(c)
4297546688      # On retrouve la référence de a
>>> c
5
\end{lstlisting}
\end{minipage}

{\bf NB :} Les types : chaînes de caractères (\lstinline{'str'}), flottants (\lstinline{'float'}), booléen (\lstinline{'bool'}), tuple (\lstinline{'tuple'})  et fonction (\lstinline{'fonction'}) sont tous non modifiables et se comporteront donc comme les entiers.


Les listes sont {\bf modifiables}, leur valeur peut être changée après leur création. Les choses se passent donc très différemment : 
...
\begin{minipage}{0.45\linewidth} 
\begin{center}
\includegraphics[width=8.5 cm]{variable_liste.pdf}
\end{center}
...
\begin{minipage}{0.45\linewidth} 
\begin{center}
\includegraphics[width=8.5 cm]{variable_liste2.pdf}
\end{center}
\end{minipage}
\medskip

Les noms \lstinline{meteo} et \lstinline{temps} pointent vers la même liste. La modification de cette liste change aussi bien la valeur de \lstinline{meteo} que de \lstinline{temps}. Il faut bien être conscient de ce comportement car il peut être la source d'erreurs. On parle d'effet de bord\footnote{C'est une traduction de \og side effect\fg\ qui signifie aussi effet secondaire.} non désiré.
...
Pour créer une copie d'une liste on peut utiliser la fonction \lstinline{list()} ou un tranchage.
...
\begin{exercice}{}
\'Ecrire un code pour permuter les éléments d'indices \lstinline{a} et \lstinline{b} de la liste \lstinline{L}.

\begin{center}
\begin{minipage}{0.95\linewidth}
\begin{lstlisting}%[escapeinside = ;;]  % définit des délimiteurs pour échapper des caractères
...
\begin{exercice}{}
Compléter l'instruction évaluée dans la console ci-dessous pour que la liste \texttt{L3} contienne alternativement les éléments des listes \texttt{L1} et \texttt{L2} de même taille.
...
Le site \url{http://pythontutor.com/visualize.html} fournit un outil de visualisation de l'état courant d'un programme très pratique. 
...
Pour copier en profondeur une liste on peut utiliser la fonction \lstinline{deepcopy()} du module \lstinline{copy}.
