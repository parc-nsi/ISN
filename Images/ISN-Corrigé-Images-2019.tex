...
\fancyhead[C]{\textbf{Chapitre 7 Images}}
...
\begin{enumerate}
	...
	\item Le  pixel blanc situé à droite de la barre centrale du F est en ligne d'index $2$ et colonne d'index $3$.
...
Le code ci-dessous génère une image, de définition $500 \times 500$, constituée de bandes noires parallèles dont les côtés coincidant avec un  bord de l'image, mesurent $50$ pixels.
...
\end{minipage}\hfill
\begin{minipage}{0.2\linewidth}
\includegraphics[scale=0.23]{ressources/bandes.png}
\end{minipage}
...
Elle fait apparaître la différence entre \texttt{(lig + col) // 50} et \texttt{lig // 50 + col // 50} qui est la seule expression différente entre les codes générant un damier ou des bandes.

Si on pose les divisions euclidiennes de \texttt{lig} et \texttt{col} par $50$, on obtient  $\text{ \texttt{lig}} = q \times 50 + r$ avec $0 \leqslant r < 50$ et  $\text{ \texttt{col}} = q' \times 50 + r'$ avec $0 \leqslant r' < 50$. Dans le code, $r$ est représenté par \verb+lig % 50+ et $r'$ par \verb+col % 50+.

Par définition de l'opérateur \verb+//+, on a $\text{lig // 50} = q$ et   $\text{col // 50} = q'$ et donc $\text{lig // 50 + col // 50} = q + q'$.

D'autre part, on a $\text{(lig + col) // 50} = q + q' + (r + r') // 50$ et puisque $0 \leqslant r + r' < 100$ on a $(r + r') // 50 = 0$ si $0 \leqslant r + r' < 50$ et $(r + r') // 50 = 1$ sinon.
...
Puisque \texttt{n} est pair dans le contexte de cet exercice on a pour tout entier \texttt{i} compris entre $0$ et \texttt{n} l'égalité entre \texttt{(j + n) // 2} et Puisque \texttt{n} est pair dans le contexte de cet exercice on a pour tout entier \texttt{i} compris entre $0$ et \texttt{n} l'égalité entre \texttt{j // 2 + n // 2} et on peut écrire une version sans structure conditionnelle :
...
\item Si on itère $9$ fois la fonction photomaton à partir de l'image  \texttt{lenagray.png}, de dimensions $512 \times 512$, on retrouve l'image initiale. 

\item Si on itère $8$ fois la fonction photomaton à partir de l'image  \texttt{lenagray-256.png}, de dimensions $256 \times 256$, on retrouve l'image initiale. 

Si on itère $7$ fois la fonction photomaton à partir de l'image  \texttt{lenagray-128.png}, de dimensions $128 \times 128$, on retrouve l'image initiale. 
...
\begin{itemize}
\item Si elle est paire, on la divise par $2$ \ie{} que si le bit de poids faible est $0$, alors on le supprime, ce qui revient à faire passer ce $0$ de la dernière à la première position dans l'écriture binaire :
\begin{equation*}
\underbrace{1.0.1.}_{n-1 \text{ bits}}\boxed{0} \quad \stackrel{\text{photomaton}} {\Longrightarrow} \quad \boxed{0}\underbrace{1.0.1.}_{n-1 \text{ bits}}
\end{equation*}
\item Si elle est impaire, on prend son quotient dans la division euclidienne par $2$ et on ajoute $2^{n-1}$. Cela revient à faire passer  le bit de poids faible égal à  $1$ de la dernière à la première position dans l'écriture binaire :
\begin{equation*}
\underbrace{\underbrace{1.0.1.}_{n-1 \text{ bits}}}_{\text{quotient par 2}}\hspace*{-10pt}\boxed{1} \quad \stackrel{\text{photomaton}}{\Longrightarrow} \quad\underbrace{\boxed{1}\underbrace{0..0}_{n-1 \text{ bits}}}_{2^{n-1}} \: + \: \underbrace{1.0.1.}_{n-1 \text{ bits}}   = \boxed{1}\underbrace{1.0.1.}_{n-1 \text{ bits}}
\end{equation*}
\end{itemize}
\end{enumerate}
\begin{tabular}{*{3}{c}}
...
\begin{center}
\fbox{Source $2^{8}\times2^{8}$ pixels}

\bigskip


\includegraphics[scale=0.25]{ressources/lenagray-256.png}
\end{center}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-1.png}
\end{center}
\end{minipage}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-2.png}
\end{center}
\end{minipage}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-3.png}
\end{center}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-4.png}
\end{center}
\end{minipage}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-5.png}
\end{center}
\end{minipage}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-6.png}
\end{center}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-7.png}
\end{center}
\end{minipage}
...
\includegraphics[scale=0.25]{ressources/lenagray-256-photomaton-iteration-8.png}
\end{center}
\end{minipage}
...
{\centering \Large \bfseries Barême
...
\begin{itemize}[label=\ding{43}]
	...
	\item \emph{\textbf{Copie manuscrite sur $4$ points :}}
		
		\begin{itemize}
			
			\item Respect de l'indentation : $\boxed{2 \text{ points}}$.
			
			\item Soin de la copie : $\boxed{1 \text{ point}}$.
			
			\item Correspondance avec le code source : $\boxed{1 \text{ point}}$.
		
		
		\end{itemize}
  
  \item \emph{\textbf{Code source sur $16$ points }:}
  
  \begin{itemize}
  
      \item \textbf{Exercice 1 sur $3,5$ points :}
      
      	\begin{itemize}
			
			\item Repérage de pixel : $\boxed{0,5 \text{ points}}$.     
			
			\item  Génération des bandes : 	$\boxed{1,5 \text{ points}}$ pour la formule.
      	
      	    \item  Fonction \verb+inverser_couleurs_pbm+ : 	$\boxed{1,5 \text{ point}}$ ($1$ pour la correction et $0,5$ pour la syntaxe).
      ...
      \item \textbf{Exercice 2 sur $6$ points :}
      
      	\begin{itemize}
			
			\item  Fonction \verb+rechercher_pixel+ : 	$\boxed{2 \text{ points}}$ ($1,5$ pour la correction et $0,5$ pour la syntaxe).   
			
				\item  Fonction \verb+flip+ : 	$\boxed{2 \text{ points}}$ ($1,5$ pour la correction et $0,5$ pour la syntaxe).   
      	
      	    	\item  Fonction \verb+flop+ : 	$\boxed{2 \text{ points}}$ ($1,5$ pour la correction et $0,5$ pour la syntaxe).   
      	
      	\end{itemize}
      	
 	\item \textbf{Exercice 3 sur $6,5$ points :}
      	\begin{itemize}
			
			\item  Fonction \verb+transformation+ : 	$\boxed{2 \text{ points}}$ ($1,5$ pour la correction et $0,5$ pour la syntaxe).   
			
				\item  Fonction \verb+photomaton+ : 	$\boxed{2 \text{ points}}$ ($1,5$ pour la correction et $0,5$ pour la syntaxe).   
      	
      	    	\item  Fonction \verb+photomaton_iterer+ : 	$\boxed{1,5 \text{ point}}$ ($1$ pour la correction et $0,5$ pour la syntaxe).   
      	
      	          \item Observation : $\boxed{0,5 \text{ points}}$.
      	          
      	           \item Conjecture : $\boxed{0,5 \text{ points}}$.
      	\end{itemize}
