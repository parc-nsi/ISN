...
\fancyhead[C]{\textbf{Variables et tests}}



%%%%%%%%%%%%%%%%%%%%%%DOCUMENT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\author{\Large Spé ISN}  % Pour spécifier l'auteur qui sera affiché sous le titre
\title{\Large Variables et tests}  % Titre 
...
\end{introduction}    
 ...
 \section{Un premier programme en Python}
 
\begin{definition}{(Langage de programmation)}
Un {\bf programme informatique} est un texte qui décrit les opérations que l'on souhaite faire exécuter par un ordinateur. Ce texte est écrit en suivant les règles d'un {\bf langage de programmation}. Dans ce cours, nous utiliserons le langage {\bf Python}  (version 3).
\end{definition}

Un langage de programmation fait interface entre l'homme et la machine, il est compréhensible par le premier et exécutable par la seconde. Un langage dont la structure est proche du fonctionnement du processeur\footnote{Le processeur est le composant de l'ordinateur qui exécute les instructions du langage machine.} est dit de bas niveau. Réciproquement, un langage de haut niveau doit être plus facile à comprendre. Python est un langage de haut niveau.
...
\item La \textbf{console }permet de saisir une instruction Python après le prompt constitué de trois chevrons \texttt{>>>} ou d'une étiquette \texttt{In [1]:} . On marque la fin de la saisie avec un retour chariot puis l'interpréteur Python donne sa réponse en-dessous.
\item \textbf{L'éditeur de texte} permet d'écrire un programme constitué de plusieurs instructions et de l'enregistrer sur le disque.
\item \textbf{La barre de menus}, permet de dérouler différents menus : nous utiliserons principalement les menus File, Edit et Run. Le menu Run propose différentes formules et les résultats de l'exécution du programme s'affichent dans la console. 

\end{itemize}

\begin{center}
\includegraphics[scale=0.5]{pyzo_fenetre.png}
\end{center}

Dans votre répertoire personnel, créez un dossier \verb+ISN+ puis à l'intérieur, un dossier pour le premier chapitre : \verb+Ch1_Variables_et_tests+.

On veillera à toujours respecter la règle suivante : les noms de fichier ou de dossier ne doivent pas comporter d'espace, d'accents ou de caractères spéciaux. Pour séparer les mots, vous pouvez utiliser \verb+-+ (tiret) ou \verb+_+ (tiret-bas ou underscore).

Dans ce dossier, créer un fichier \verb+Devinette.py+ et reproduire le code qui suit.
...
print('Bravo, vous avez trouv;\'e; !')
\end{lstlisting}

Sauver puis exécuter le code à l'aide du menu \verb!Run! (noter le raccourci clavier au passage).
\medskip

Que fait ce programme ? \dotfill

\lignes{2}

Commenter la ligne 1 en la faisant précéder du caractère \verb+#+. Exécuter le programme, que se passe-t-il ?

\lignes{3}

Que se passe-t-il à l'exécution de l'instruction \lstinline{n = randint(1, 100)} ? De quel type est-elle ?

\lignes{3}

Quel est le rôle de l'instruction \lstinline{print('Python vient de choisir un nombre entre 1 et 100.')} ?

\lignes{2}

Que permet l'instruction \lstinline{r = int(input('Devinez-le : '))} ?
...
int main()
{
    int n = rand() % 100 + 1;
    int r;
    cout << "C++ vient de choisir un nombre entre 1 et 100." << endl;
    cout << "Devinez-le : ";
    cin >> r;
    while (r != n){
        if (r < n) {
...
Quelles différences de syntaxe entre \texttt{Python} et \texttt{C++} apparaissent de façon significative ?


\lignes{7}


\begin{exercice}{Suite d'instructions}
...
\begin{minipage}{0.5\linewidth}
\begin{center}
	\includegraphics[scale=0.47]{cavern.png}
\end{center}
\end{minipage}
...
\begin{itemize}[label=\ding{43}]
	\item  \verb+droite()+ : elle fait un quart de tour sur sa droite.
	\item \verb+gauche()+ : elle fait un quart de tour sur sa gauche.
	\item \verb+fonce()+ : elle va tout droit jusqu'à rencontrer un mur.
...
\section{Variables et affectation}

Pour stocker et manipuler les informations élémentaires on utilise des {\bf variables} qui associent à un nom une valeur : une variable est une boîte qui porte un nom. 
\smallskip

Pour donner une valeur à une variable on utilise une instruction fondamentale : {\bf l'affectation}. Sa syntaxe utilise le  symbole = sous la forme :
\medskip


\begin{minipage}{0.6\linewidth} %début texte
\begin{center}
%{\LARGE {$nom\_variable = \begin{array}{c}\text{valeur ou} \\ \text{expression}\end{array}$}}
{\LARGE \lstinline{nom_variable = expression}}
\end{center}
\end{minipage}% fin texte
\hfill 
\begin{minipage}{0.35\linewidth} 
\begin{center}
\includegraphics[width=5 cm]{fig_variable.pdf}
\end{center}
\end{minipage}
\medskip

{\bf Attention}, le symbole \verb+=+ n'a rien à voir avec le égal des mathématiques, il n'est pas symétrique. En pseudo-code il s'écrit 
par exemple \verb+a+ $\leftarrow$ \verb+5+. Permuter les deux parties donne lieu à une erreur : 
\begin{lstlisting}
>>> 5 = a
...
$\bullet$ Le {\bf nom de la variable} peut comporter plus d'une lettre mais il faut respecter quelques règles : 
\begin{itemize}
  \item On peut utiliser les lettres minuscules ou majuscules (a $\rightarrow$ z et A $\rightarrow$ Z) qui sont différentiées ainsi que les 10 chiffres (0 $\rightarrow$ 9). Les autres caractères sont interdits sauf le tiret bas \_ (mais pas le tiret usuel - car c'est le symbole de la soustraction).
  \item Le premier caractère est obligatoirement une lettre.
  \item Les mots réservés du langage sont interdits :
  \begin{lstlisting}
  >>> else = 5
SyntaxError: invalid syntax
  \end{lstlisting}
  \item Utiliser un nom de fonction ne provoquera pas d'erreur directe mais c'est absolument à éviter :
\begin{lstlisting}
>>> print('Hello Word')
Hello Word
>>> print = 1789
...
$\bullet$ {L'expression} est évaluée et donne une {\bf valeur} qui est affectée à la variable en écrasant une éventuelle valeur précédente. 
\begin{lstlisting}
>>> a = 5
>>> a
5
...
Une {\bf expression} est une combinaison de valeurs explicites (appelées littéraux), de variables, d'opérateurs et de fonctions qui est évaluée pour donner lieu à une valeur.
...
>>> y
1 
>>> (x, y)     # Pour afficher les valeurs de plusieurs variables
(100, 1)
\end{lstlisting}
\end{exercice}
...
\end{center} \end{minipage}% fin texte
\hfill 
...
\begin{definition}{(\'Etat courant d'un programme)}
...
\hline
ligne & \multicolumn{3}{c|}{état courant après }  \\\hline
 & \ \ \ a \ \ \ & \ \ \ b \ \ \ & \ \ \ c \ \ \ \\ \hline
1 &  &  &  \\ \hline
 &  &  &  \\ \hline
 &  &  &  \\\hline
...
Le site \url{http://www.pythontutor.com/} vous permet de visualiser l'état courant de vos programmes.
...
\hline
\hline
\verb+int+ & entier & $1871$ \\
\hline
\verb+float+ & flottant (décimal) & $3.1415$ \\
\hline
\verb+bool+ & booléen (\verb+True+ ou \verb+False+) & \verb+True+ \\
\hline
\verb+tuple+ & $n$-uplet & \verb+(2, 0, 1, 7)+ \\
\hline
\verb+str+ & chaîne de caractères (string) & \verb+'ISN'+ \\
\hline
\verb+list+ & liste & \verb+[9, 8, 3.14, 'Pi']+\\
\hline
\verb+function+ & fonction & \verb+max+\\
\hline
\end{tabular}
\end{center}

On obtient le type d'une variable par la fonction \lstinline{type} : 
\bigskip
...
\begin{center}
{\bf\large Quelques opérateurs usuels}
\end{center} 
...
{\bf Opérateur} & {\bf Type} & {\bf Action} \\\hline \hline
+ & int, float & Addition \\\hline 
- & int, float & Soustraction \\\hline 
* & int, float & Multiplication \\\hline 
** & int, float & Puissance \\\hline
/ & float & Division décimale \\\hline
 // & int & Quotient de la division euclidienne \\\hline
 \% & int & Reste de la division euclidienne \\\hline
+ & str & Concaténation de deux chaines de caractères \\\hline
not & bool  & Négation (False $\leftrightarrow$ True)\\\hline
and & bool  & Et logique \\\hline
or & bool  & Ou  logique\\\hline
...
Si aucune des conditions présentes derrière un des mot-clé \verb+elif+ n'est évaluée à True alors le dernier bloc est exécuté. 

Remarque : Un seul des blocs d'instructions peut être exécuté : le premier possible ceci même si l'état courant rend plusieurs des conditions valides (\verb+True+).

Les conditions sont souvent construites à l'aide des opérateurs de comparaison.

\begin{center}
\begin{tabular}{|c|c|}\hline 
Opérateur & test effectué \\\hline \hline
== & égalité \\\hline 
!= & différent \\\hline
<= & inférieur ou égal \\\hline
>= & supérieur ou égal \\\hline
< & inférieur strict \\\hline 
> & supérieur strict \\\hline
\end{tabular}
\end{center}
...
\noindent{\bf Tester ou ne pas tester un booléen ?}

Supposons que l'on dispose d'une fonction \lstinline{est_premier(n)} qui renvoie le booléen True si n est premier et False sinon. Que pensez-vous des deux codes suivants : 
...
L'écriture de gauche est une maladresse (coupable ?) qui revient à la formulation : \og Si il est vrai que n est premier alors ...\fg. On préfère simplement \og Si n est premier alors ....\fg.

\begin{exercice}{}

\'Ecrire un programme qui affiche le plus grand de deux nombres saisis par l'utilisateur (sans utiliser la fonction \lstinline{max}).
\end{exercice}
...
La conjecture de Syracuse étudie la fonction qui à un entier $n$ associe $n/2$ si $n$ est pair et $3*n+1$ s'il est impair. \'Ecrire un programme qui affiche l'image d'un nombre $n$ saisi par l'utilisateur.
\end{entrainement}


\section{Ressources en ligne et installation de Python }

Quelques liens vers des ressources  : 
\begin{itemize}
{\small
\item Site officiel : \url{https://www.python.org/}   \quad  \url{https://www.python.org/download/}  \quad 

\url{https://www.python.org/doc/}
\item Les bases de  Python pour le lycée : \url{http://www.frederic-junier.org/PythonSeconde/Python_Seconde_Parc/accueil_python_2nde.html}
\item Tutoriel sur Python : {http://www.tutorialspoint.com/python3/index.htm}
\item Le site de l'Olympiade française d'informatique (France-ioi) : 
\url{http://www.france-ioi.org/algo/chapters.php}
}
\end{itemize}
\medskip

 Python est installé par défaut sur la distribution Linux Ubuntu, sous Windows ou MAC OSX on peut télécharger les installateurs à partir de \url{http://www.python.org/download/} et on disposera d'un environnement graphique Idle avec console et éditeur de texte.
Anaconda est une distribution plus complète (avec tous les module et bibliothèques nécessaires) qui se télécharge depuis \url{https://www.anaconda.com/distribution/}. Anaconda intègre plusieurs IDE comme Idle et Spyder mais aussi le notebook Jupyter.

Tout interpréteur Python peut être associé à l'IDE  Pyzo qui se télécharge et s'installe très facilement depuis\url{https://pyzo.org/}.

Il existe aussi des interpréteurs en ligne comme \url{https://repl.it/languages/python3}.
