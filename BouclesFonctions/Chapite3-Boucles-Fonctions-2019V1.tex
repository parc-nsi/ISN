...
\title{\Large Chapitre 3 : Boucles et fonctions}  % Titre 
...
\section{Boucle inconditionnelle}
...
3
 ...
 Ici la fonction range renvoie un \emph{itérateur} qui produit consécutivement les valeurs entières de 0 à 5. La boucle \lstinline{for} ne fait que parcourir les valeurs de cet itérateur.  
...
En 2020, le mois de juin commence un lundi. \`A l'aide de deux boucles imbriquées, faire afficher les jours des quatre premières semaines du mois sous la forme :
\begin{verbatim}
lundi 1 mai
mardi 2 mai
.
...
Lorsqu'on a besoin de réutiliser tout un bloc  d'instructions, on peut l'encapsuler dans une \textbf{fonction}. On étend ainsi le langage avec une nouvelle instruction. Une fonction sert à factoriser du code, à rendre le programme plus lisible et plus facile à maintenir et à partager. C'est un outil de modularité. 

Pour déclarer une fonction, on définit son \textbf{en-tête} avec  son \textbf{ nom} et des \textbf{paramètres formels} d'entrée. Viennent ensuite le bloc d'instructions et  une valeur de retour. En Python on définit une fonction ainsi :
...
Par exemple une fonction \texttt{carre} qui prend en paramètre un nombre \texttt{x} et qui retourne son carré, s'écrira :
...
Une fonction peut prendre plusieurs paramètres. Par exemple une fonction \verb+carre_distance_origine(x,y)+ qui prend en paramètres deux nombres \texttt{x} et \texttt{y} et qui retourne le carré de la distance d'un point de coordonnées \texttt{(x, y)} à l'origine d'un repère orthonormal, s'écrira : 
...
Nous venons de voir la définition d'une fonction. On exécute la fonction en substituant aux \textbf{paramètres formels} des valeurs particulières appelées \textbf{arguments}. Cette instruction se nomme un \textbf{appel de fonction} :
...
\begin{enumerate}
	\item Écrire une fonction \texttt{aumoinsun(a,b,c)} qui retourne un booléen indiquant si l'un au moins des nombres \texttt{a, b} ou \texttt{c} est positif.
	
	\lignes{3}
	
	
	\item Écrire une fonction \texttt{tous(a,b,c)} qui retourne un booléen indiquant si tous les  nombres \texttt{a, b, c} sont positifs.
	
		\lignes{3}
		
		
		\item Écrire une fonction \texttt{croissant(a,b,c)} qui retourne un booléen indiquant si \texttt{a, b, c} sont dans l'ordre croissant.
	...
	Écrire une fonction \texttt{bissextile(a)} qui retourne un booléen indiquant si l'année \texttt{a} est bissextile.
...
\begin{entrainement}{}
Écrire une fonction \texttt{nombreRacinesTrinome(a,b,c)} qui retourne le nombre de racines distinctes d'un trinôme $T:x \mapsto ax^{2}+bx+c$.
...
Fonction & Effet \\ 
\hline 
\texttt{randrange(a,b)} & retourne un entier aléatoire dans [a;b[ \\ 
\hline 
\texttt{randint(a,b)} & retourne un entier aléatoire dans [a;b] \\ 
\hline 
\texttt{random()} & retourne un décimal aléatoire dans [0;1[ \\ 
\hline 
\texttt{uniform(a,b)} & retourne un décimal aléatoire dans [a;b] \\ 
\hline 
\end{tabular} 
...
\begin{enumerate}
	\item Écrire une fonction \texttt{sommeDe(n)} qui retourne la somme des résultats obtenus en lançant \texttt{n} dé à $6$ faces.
	
	\lignes{4}
	
	\item Écrire une fonction \texttt{urne()} qui retourne le numéro de la boule tirée dans une urne qui contient cinq boules numérotées $1$, trois boules numérotées $2$ et deux boules numérotées $3$.
...
Remarque : Pour faire une opération jusqu'à obtenir une certaine condition il suffit d'ajouter l'opérateur logique \lstinline{not()}. 
...
\begin{enumerate}
  \item \'Ecrire un programme équivalent au programme  \fbox{1} à l'aide d'une boucle \lstinline{while}.
  
  \lignes{5}
  
  \item  La suite de Syracuse est une suite d'entiers naturels définie de la manière suivante : on part d'un nombre entier plus grand que zéro ; s'il est pair, on le divise par 2 ; s'il est impair, on le multiplie par 3 et on ajoute 1. Une conjecture non démontrée stipule que la suite atteint $1$ quel que soit le premier terme. Écrire une fonction \texttt{tempsvol(n)} qui retourne le rang du premier terme égal à $1$  de la suite de Syracuse si le terme de rang $0$ est \texttt{n}
...
Les opérateurs  \verb+//+ et \verb+%+ permettent de calculer respectivement le quotient et le reste de la division euclidienne de deux entiers.
...
\begin{enumerate}
  ...
  \item Avec l'algorithme d'Euclide déterminer le Plus Grand Commun Diviseur des entiers 1050 et 315.
  
   \lignes{4}
   
  \item  Écrire une fonction \texttt{euclide(a, b)} qui retourne le PGCD des entiers \texttt{a} et \texttt{b} passés en paramètres.
