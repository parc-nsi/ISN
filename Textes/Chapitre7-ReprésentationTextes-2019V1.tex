...
\fancyhead[C]{\textbf{Représentation des textes}}
...
\title{\Large Chapitre 6 : Représentation des textes} % Titre 
\date{\small Année scolaire 2019/2020} %en utilisant \today on obtiendra la date courante lors de la production du document
...
\item Une chaîne de caractères est une séquence éventuellement vide de caractères. Chaque caractère est uen chaîne de longueur 1.
...
\item Les chaînes de caractères partagent certaines propriétés avec les listes :
\begin{itemize}
\item Une chaîne de caractères \texttt{c} peut être vue comme un tableau de caractères indexés de 0 à \texttt{len(c) - 1} de gauche à droite et de \texttt{-1} à \texttt{-len(c)} de droite à gauche. L'opérateur crochet permet d'accéder au caractère d'index \texttt{i} avec la syntaxe \texttt{c[i]} ou à la tranche de caractères d'index appartenant à l'intervalle $\Interfo{\text{\texttt{i}}}{\texttt{j}}$  avec la syntaxe \texttt{c[i:j]} si \texttt{0 $\leqslant$ i < j} ou \texttt{c[i:j:-1]} si \texttt{i < j $\leqslant$ -1}.
...
\begin{enumerate}
	\item Recopier et compléter le code de la  fonction \texttt{miroir} qui prend en paramètre une chaîne de caractères, pour qu'elle  retourne la chaîne de caractères renversée :
\begin{lstlisting}
def miroir(chaine):
    res = ''
    for c in chaine:
...
\item Écrire une fonction \texttt{palindrome(chaine)} qui retourne un booléen indiquant si la chaîne de caractères est un palindrome. Cette fonction ne doit pas tenir compte des espaces.
...
\begin{definition}{(Table de codage)}
 \textbf{Une table de codage} ou \textbf{jeu de caractères} ou  \textbf{charset} ou  \textbf{code point} permet de faire correspondre les    caractères qu'on souhaite coder à  un unique code.
...
La table de codage \textbf{ASCII} représentée ci-dessous contient :

\begin{itemize}
\item  les lettres de l'alphabet latin en majuscule et minuscule et les chiffres de $0$ à $9$ ;
\item des signes de ponctuation et es opérateurs arithmétiques ;
...
\begin{exercice}{}
En \texttt{Python}, la fonction \texttt{ord} associe à un caractère son code Unicode et la fonction \texttt{chr} est sa réciproque :
...
\begin{enumerate}
	\item Définir en quelques lignes le chiffrement d'une chaîne de caractères par l'algorithme \texttt{rot13}.
...
Les caractères de la table \textbf{ASCII} se sont vite avérés insuffisants pour l'encodage de textes dans d'autres langues que l'Anglais qui ne comprend pas de caractères accentués par exemple. Pour y remédier, l'\textbf{ISO} (\textit{Organisation internationale de normalisation}) a proposé la norme \textbf{ISO-8859} qui   étend la table \textbf{ASCII} en utilisant le huitième bit. Sur un octet, on peut ainsi représenter deux fois plus de caractères, soit $2^{8}=256$. 

La norme \textbf{ISO 8859} comprend seize tables de codages, certes compatibles entre elles et avec ASCCI sur les $128$ premiers caractères, mais avec $256$ caractères il n'est pas possible d'écrire un texte avec un mélange de caractères arabes et russes. 
...
Pour assurer l'universalité de la représentation des caractères, l'\textbf{ISO} a  défini une table de codage universelle, l'\textit{Universal Character Set} sous la norme \textbf{ISO-10646}. Cette norme associe à chaque caractère un unique nombre appelé \textbf{point de code} et permet de représenter $2^{32}=\np{4294967296}$ caractères. Actuellement, $\np{110000}$ caractères sont recensés.
...
\end{lstlisting}
\item Écrire un code \texttt{Python} qui affiche tous les caractères dont le point de code est compris entre \texttt{U+1F600} et \texttt{U+Ff64F}.
  \end{enumerate}
...
\hline 
\texttt{U+008F} (128) à \texttt{U+07FF} (2047)  & \texttt{110xxxxx} \texttt{10xxxxxx} & $11$ bits \\ 
\hline 
...
\begin{enumerate}
	...
	\item Sur combien de caractères sont encodés les caractères accentués en \textbf{UTF-8} ?
	...
	\item Déterminer l'encodage \texttt{UTF-8} en donnant les octets en décimal, en binaire et en hexadécimal des caractères suivants :
		...
		\end{enumerate}
	\item Écrire en \texttt{Python} une fonction  \texttt{longueur(b)} qui parcours une chaîne d'octets de type \texttt{bytes} encodée en \textbf{UTF-8} et qui retourne le nombre de caractères représentés.
