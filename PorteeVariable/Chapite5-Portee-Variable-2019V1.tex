...
\title{\Large Chapitre 5 : Portée d'une variables, passage d'argument à une fonction}  % Titre 
...
\end{center}
 ...
 \begin{enumerate}
 	\item Que se passe-t-il lorsqu'on exécute les codes ci-dessous ? Commenter.
...
\end{programme}
 ...
 \item Que se passe-t-il lorsqu'on exécute les codes ci-dessous ? Commenter en précisant la portée des variables \texttt{a}, \texttt{b} et \texttt{c} lors de chaque exécution de l'instruction \texttt{print(a, b, c)}.
...
\begin{exercice}{}
Que se passe-t-il lorsqu'on exécute les codes ci-dessous ? Commenter en précisant la portée de la  variable \texttt{a} lors des appels successifs des fonctions \texttt{incremente1(a)}, \texttt{incremente2()} puis \texttt{incremente3()}.
...
\end{exercice}
 ...
 \section{Passage d'argument à une fonction}
...
def fonction(parametre1, parametre2):
	bloc de la fonction
\end{lstlisting}
 Lorsque la définition d'une fonction comporte des paramètres, lors de l'appel de cette fonction on doit assigner à chaque \textbf{paramètre} une valeur obtenue par évaluation d'une expression calculée à partir de littéraux et de variables. Les valeurs effectives transmises à la fonction lors de l'appel sont appelées des \textbf{arguments}. On crée alors pour chaque argument variable locale à la fonction référencée par le nom du paramètre formel.
 
Lorsqu'on passe comme argument une variable, on distingue habituellement  deux types de passage d'argument :
 
 \begin{itemize}
 
	\item Le \textbf{passage par valeur} : le paramètre formel reçoit une copie de la variable passée en argument. Dans ce cas une modification subie dans la fonction par la  variable locale créée lors de l'appel ne se répercute pas sur la variable passée en argument.
	
	\item Le \textbf{passage par référence} : le paramètre formel reçoit une référence vers la valeur référencée par la variable passée en argument. Dans ce cas une modification subie dans la fonction par la  variable locale créée lors de l'appel  se répercute  sur la variable passée en argument.  Le passage par référence permet d'économiser de l'espace mémoire mais peut se traduire par des effets de bord.
 \end{itemize}
 
 En \texttt{Python}, on a vu qu'il existait des \textbf{objets non modifiables} comme les types de base (\texttt{int}, \texttt{float}, \texttt{bool}) ou les collections de type \texttt{tuple} et des \textbf{objets  modifiables} comme les collections de type \texttt{list} ou \texttt{dict}. 
 
En \texttt{Python} tous les passages d'argument se font par référence, même si pour les objets non modifiables, le comportement est le même que pour un passage par valeur. Lors du passage d'une variable en argument, le paramètre formel reçoit la référence de cette variable. Si l'objet référencé est modifiable (une liste, un dictionnaire), il pourra être modifié par la fonction, sinon  (pour un entier, un flottant \ldots) il ne pourra pas être modifié. 
 
 \end{definition}
 
 
 \begin{exemple}{Passage en argument d'une variable référençant un objet non modifiable}
 On donne ci-dessous un exemple de passage d'une variable référençant un entier non modifiable à une fonction, on peut observer que même si la variable globale et la variable locale à la fonction portent le même nom \texttt{x}, la variable locale disparaît à la fin de l'exécution de la fonction sans laisser de trace sur la variable globale. Testez en ligne sur \href{http://pythontutor.com/visualize.html#code=def%20incremente%28x%29%3A%0A%20%20%20%20x%20%3D%20x%20%2B%201%0A%20%20%20%20%0Ax%20%3D%204%20%0Aincremente%28x%29&cumulative=false&curInstr=0&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false}{PythonTutor}.
 ...
 \includegraphics[scale=0.3]{images/passage_immutable.png}
 \end{center}
 \end{minipage}\hfill
   \begin{minipage}{0.45\linewidth}
   \begin{center}
  ...
  \includegraphics[scale=0.3]{images/passage_immutable1.png}
  \end{center}
 \end{minipage}
 ...
 \bigskip
  ...
  \includegraphics[scale=0.3]{images/passage_immutable3.png}
  \end{center}
 \end{minipage}
 \end{center}
 ...
 \begin{exemple}{Passage en argument d'une variable référençant un objet modifiable}
 On donne ci-dessous un exemple de passage d'une variable référençant une liste   modifiable à une fonction, on peut observer l'effet de bord produit par chaque exécution de la fonction sur la liste passée en paramètre. Testez en ligne sur \href{http://pythontutor.com/visualize.html#code=def%20ajout_liste%28L,%20a%29%3A%0A%20%20%20%20L.append%28a%29%0A%20%20%20%20%0AL%20%3D%20%5B%5D%0Aajout_liste%28L,%201%29&cumulative=false&curInstr=0&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false}{PythonTutor}.
 ...
 \includegraphics[scale=0.3]{images/passage_mutable1.png}
 \end{center}
 \end{minipage}\hfill
   \begin{minipage}{0.45\linewidth}
   \begin{center}
  ...
  \includegraphics[scale=0.3]{images/passage_mutable2.png}
  \end{center}
 \end{minipage}
 ...
 \bigskip
  ...
  \includegraphics[scale=0.3]{images/passage_mutable3.png}
  \end{center}
 \end{minipage}
...
\begin{enumerate}
	\item Écrire une fonction \texttt{echange\_valeurs(L, a, b)} qui prend en paramètre une liste \texttt{L} et deux entiers \texttt{a} et \texttt{b} et qui permute par effet de bord les valeurs de la liste \texttt{L} en positions \texttt{a} et \texttt{b}. On commencera par vérifier que \texttt{a} et \texttt{b} sont bien des index valides pour la liste.
	
\lignes{3}

\item Écrire une fonction \texttt{carre\_liste(L)} qui prend en paramètre une liste \texttt{L} et qui modifie par effet de bord la liste \texttt{L} en remplaçant chacun de ses éléments par leur carré. 
	
\lignes{3}
	
	
\item Écrire une fonction \texttt{carre\_liste2(L)} qui prend en paramètre une liste \texttt{L} et qui retourne une nouvelle liste constituée des carrés des éléments de  \texttt{L}, sans modifier la liste \texttt{L}.
	
\lignes{3}

\item Écrire une fonction \texttt{pair\_impair(L)} qui prend en paramètre une liste \texttt{L} d'entiers et qui retourne une nouvelle liste dont constituée d'abord des éléments pairs   de \texttt{L} puis des éléments impairs de \texttt{L}.
	
\lignes{4}
	
\item Écrire une fonction \texttt{pair\_impair2(L)} qui prend en paramètre une liste \texttt{L} d'entiers et qui modifie en plaçant tous les éléments pairs de  \texttt{L} au début de la liste et  tous les  éléments impairs de \texttt{L} à la fin de la liste.
